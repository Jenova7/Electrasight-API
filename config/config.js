...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.Electra-insight');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\Electra\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/Electra/';
  if (isLinux) dataDir = process.env.HOME + '/.electra/';
}
...
var bitcoindConf = {
  ...
  user: process.env.BITCOIND_USER || 'Jeno',
  pass: process.env.BITCOIND_PASS || 'INeedARevolutionANewDirection',
